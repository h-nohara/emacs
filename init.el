...
;; (set-default-font "-1ASC-Liberation Mono-normal-normal-normal-*-*-*-*-*-m-0-iso10646-1") ;; comment out
...
(setq gc-cons-threshold (* 200 1024 1024))  ;; ロードが遅いときはメモリの閾値が低すぎるのが原因かも
...
(setq helm-ag-base-command "rg --vimgrep --no-heading -i") ;; apt install ripgpreg
...
;;; sudo python3 -m pip install jedi==0.17.2 && sudo pip install epc
;;; If install jedi==0.18, error occurs on emacs
(defun my-load-jedi ()
  (interactive)
  (require 'epc)
  (require 'jedi-core)
...
;; Install
;; 1, M-x package-list-packages -> install flymake-python-pyflakes
;; 2, pip install pyflakes
;; 3, Add (add-hook 'python-mode-hook 'flymake-python-pyflakes-load)
...
(custom-set-variables
 ...
 '(package-selected-packages
   '(graphql-mode vue-mode flymake-diagnostic-at-point flymake-python-pyflakes flymake svg dockerfile-mode yaml-mode protobuf-mode impatient-mode emmet-mode helm-ls-git iedit typescript-mode highlight-unique-symbol paren-completer flycheck helm-projectile htmlize yasnippet-snippets yasnippet elpy company-jedi counsel neotree xclip lsp-ui lsp-mode company-go go-mode magit edit-server restclient helm-swoop helm-migemo migemo dumb-jump swift-mode color-moccur git-gutter highlight-symbol helm-tramp smooth-scrolling company-irony irony easy-kill helm-git-grep helm-ag web-mode js2-mode company-tern))
